...
export interface DataliftErrorDetails extends ErrorDetailsApi, Details {}
...
export type ErrorDetails =
  ...
  | ResourceInfo
  | Help
  | DataliftErrorDetails
  | any;

export interface DataliftError extends Error {
...
/**
 * Construct a DataliftError from an AxiosError.
 *
 * @param clientError A client error object.
 */
const grpcResponseToError = (clientError: AxiosError): DataliftError => {
  ...
  if (response.data.details !== undefined && response.data.details.length > 0) {
    ...
    const details = response.data.details.map((detail: any) => {
