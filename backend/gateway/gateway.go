...
import (
	...
	gatewayv1 "github.com/DataliftHQ/datalift/backend/config/gateway/v1"
	"github.com/DataliftHQ/datalift/backend/gateway/meta"
	"github.com/DataliftHQ/datalift/backend/gateway/mux"
	"github.com/DataliftHQ/datalift/backend/gateway/stats"
	"github.com/DataliftHQ/datalift/backend/middleware"
	"github.com/DataliftHQ/datalift/backend/middleware/accesslog"
	"github.com/DataliftHQ/datalift/backend/middleware/errorintercept"
	"github.com/DataliftHQ/datalift/backend/middleware/timeouts"
...
// such as services, modules and resolver. This is used when constructing the stats namespace for
// a given component.
...
func RunWithConfig(f *Flags, cfg *gatewayv1.Config, cf *ComponentFactory, assets http.FileSystem) {
	...
	for _, modCfg := range cfg.Modules {
		...
		mod, err := factory(modCfg.TypedConfig, logger, scope.SubScope(
			strings.TrimPrefix(modCfg.Name, componentPrefix)))
		if err != nil {
